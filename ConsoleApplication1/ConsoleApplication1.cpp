...
class Tetragon : public Figure { //Клас-спадкоємець 
...
public:
	...
	void create_figure() { //Функція зображення фігури

		cout << "Введіть координати чотирикутника:\n";
		cout << "x1 = ";
		cin >> x1;
		cout << "y1 = ";
		cin >> y1;
		...
		cout << "y4 = ";
		cin >> y4;
		cout << "Введіть координати центру чотирикутника:\n";
		cout << "x0 = ";
		cin >> x0;
		cout << "y0 = ";
		cin >> y0;
	...
	void rotate_figure() { //Функція повороту фігури на певний кут
	...
	void move_figure() { //Функція переміщення фігури на заданий вектор

		int x, y;
		double x_end, y_end;

		cout << "Введіть координати вектора:\n";
		cout << "x = ";
		cin >> x_end;
		cout << "y = ";
		cin >> y_end;
	...
	void scale_figure() { //Маштабування фігури
	...
	void destroy_figure() { //Функція знищення зображення
		cleardevice();
		getch();
	}
...
int main() {
	...
	Figure** figure = new Figure*[3]; //Визначаємо масив вказників на абстрактний клас
	figure[0] = new Tetragon(); //Вказуємо на похідний клас
	figure[0]->create_figure(); //Створюємо чотирикутник
	figure[0]->move_figure(); //Переміщуємо фігуру на заданий вектор
	figure[0]->setScaleFactor(5); //Задаємо масштаб збільшення
	figure[0]->scale_figure(); //Масштабируем фигуру
	figure[0]->setRotateAngle(360); //Зазначаємо кут повороту у градусах
	figure[0]->rotate_figure(); //Повертаємо фігуру
	figure[0]->setScaleFactor(0.5); //Задаємо масштаб зменшення 
	figure[0]->scale_figure(); //Масштабуємо фігуру
